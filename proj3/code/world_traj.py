...
class WorldTraj(object):
    ...
    def __init__(self, world, start, goal):
        ...
        self.resolution = np.array([0.4, 0.4, 0.4])
        self.margin = 0.5

        # assume the velocity is proportional to distance between waypoints
        self.a = 3.5
        ...
        def remove_redundancy(original_path):
            ...
            # new version: only keep points between which there aren't obstacles
            return sparse_path

        # sparse_path_index = np.arange(1,len(self.path)-1,2)
        # list_sparse_path_index = list(sparse_path_index)
        # list_sparse_path_index.insert(0,0)
        # list_sparse_path_index.append(-1)
        ...
        if self.num_wayPoints > 1:
                ...
                time = np.sqrt(distance/self.a).flatten()
                # time_of_segment = np.sqrt(2*distance/self.a).flatten()
                time[0] = 1
    ...
    def update(self, t):
        ...
        if self.num_wayPoints > 1:
            ...
            time_of_segment = np.sqrt(distance/self.a).flatten()
            # time_of_segment = np.sqrt(2*distance/self.a).flatten()
            time_of_segment[0] = 1
